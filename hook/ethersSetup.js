...
export const useContracts = () => {
  ...
  // ✅ Function to set up provider & signer properly
  const setupProvider = async () => {
    try {
      ...
      if (connector && connector.connected) {
        walletAddr = await connectWallet(); // Fetch wallet address from WalletConnect
        ...
        signerInstance = await providerInstance.getSigner();
        walletAddr = await signerInstance.getAddress().catch(() => signerInstance.address); // ✅ FIXED: Prevent ENS resolution errors
      }
